import type { PickupOrder, ReturnRequest } from "./types"

export const mockOrders: PickupOrder[] = [
  // ── 1. Waiting for Arrival · 입고 미완료 (No Inbound) ──
  {
    id: "1",
    orderNumber: "ON_20260328001",
    sequenceNumber: 1,
    customerName: "박지훈",
    customerPhone: "010-3456-7890",
    customerEmail: "jihoon.park@email.com",
    orderDate: "2026-03-28 09:30",
    pickupDate: "2026-04-09 12:00",
    outboundDate: "2026-04-01 15:00",
    inboundDate: undefined,
    items: [
      { id: "item1", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
    ],
    subtotal: 299000, discount: 10000, totalAmount: 289000, paidAmount: 289000,
    paymentMethod: "네이버페이",
    status: "pending", pickupStatus: "waiting", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260328-001",
  },
  {
    id: "2",
    orderNumber: "ON_20260329002",
    sequenceNumber: 2,
    customerName: "최유진",
    customerPhone: "010-4567-8901",
    customerEmail: "yujin.choi@email.com",
    orderDate: "2026-03-29 16:00",
    pickupDate: "2026-04-14 12:00",
    outboundDate: "2026-04-04 11:00",
    inboundDate: undefined,
    items: [
      { id: "item2", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002" },
      { id: "item3", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 1, unitPrice: 45000, totalPrice: 45000, barcode: "8892841003" },
    ],
    subtotal: 495000, discount: 0, totalAmount: 495000, paidAmount: 495000,
    paymentMethod: "신용카드",
    status: "pending", pickupStatus: "waiting", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260329-002",
  },
  {
    id: "3",
    orderNumber: "ON_20260330003",
    sequenceNumber: 3,
    customerName: "노건우",
    customerPhone: "010-1928-3746",
    customerEmail: "gunwoo.noh@email.com",
    orderDate: "2026-03-30 11:20",
    pickupDate: "2026-04-12 12:00",
    outboundDate: "2026-04-03 09:00",
    inboundDate: undefined,
    items: [
      { id: "item4", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001" },
    ],
    subtotal: 189000, discount: 0, totalAmount: 189000, paidAmount: 189000,
    paymentMethod: "토스",
    status: "pending", pickupStatus: "waiting", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260330-003",
  },

  // ── 2. Pickup Available · 픽업일 기준 (입고 완료, 픽업일 이내) ──
  {
    id: "4",
    orderNumber: "ON_20260326004",
    sequenceNumber: 4,
    customerName: "김민수",
    customerPhone: "010-1234-5678",
    customerEmail: "minsu.kim@email.com",
    orderDate: "2026-03-26 14:00",
    pickupDate: "2026-04-22 12:00",
    outboundDate: "2026-03-30 10:00",
    inboundDate: "2026-04-02 09:00",
    items: [
      { id: "item5", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
      { id: "item6", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 1, unitPrice: 35000, totalPrice: 35000, barcode: undefined },
    ],
    subtotal: 334000, discount: 20000, totalAmount: 314000, paidAmount: 314000,
    paymentMethod: "신용카드",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260326-004",
  },
  {
    id: "5",
    orderNumber: "ON_20260327005",
    sequenceNumber: 5,
    customerName: "이서연",
    customerPhone: "010-2345-6789",
    customerEmail: "seoyeon.lee@email.com",
    orderDate: "2026-03-27 11:00",
    pickupDate: "2026-04-24 12:00",
    outboundDate: "2026-03-31 14:00",
    inboundDate: "2026-04-04 10:00",
    items: [
      { id: "item7", productName: "URBAN WALKER-X", sku: "2200100", quantity: 2, unitPrice: 189000, totalPrice: 378000, barcode: "8892840001" },
    ],
    subtotal: 378000, discount: 0, totalAmount: 378000, paidAmount: 378000,
    paymentMethod: "카카오페이",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260327-005",
  },
  {
    id: "6",
    orderNumber: "ON_20260330006",
    sequenceNumber: 6,
    customerName: "정다현",
    customerPhone: "010-5678-9012",
    customerEmail: "dahyun.jung@email.com",
    orderDate: "2026-03-30 10:00",
    pickupDate: "2026-04-30 12:00",
    outboundDate: "2026-04-02 09:00",
    inboundDate: "2026-04-06 14:00",
    items: [
      { id: "item8", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
      { id: "item9", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 1, unitPrice: 35000, totalPrice: 35000, barcode: undefined },
    ],
    subtotal: 334000, discount: 15000, totalAmount: 319000, paidAmount: 319000,
    paymentMethod: "토스",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260330-006",
  },
  {
    id: "7",
    orderNumber: "ON_20260331007",
    sequenceNumber: 7,
    customerName: "한소희",
    customerPhone: "010-6789-0123",
    customerEmail: "sohee.han@email.com",
    orderDate: "2026-03-31 13:00",
    pickupDate: "2026-04-27 12:00",
    outboundDate: "2026-04-03 10:00",
    inboundDate: "2026-04-06 11:00",
    items: [
      { id: "item10", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001" },
    ],
    subtotal: 189000, discount: 0, totalAmount: 189000, paidAmount: 189000,
    paymentMethod: "신용카드",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260331-007",
  },

  // ── 3. Pickup Available · 입고일 기준 (입고가 픽업일 이후 — 경고 하이라이트) ──
  {
    id: "8",
    orderNumber: "ON_20260327008",
    sequenceNumber: 8,
    customerName: "강민재",
    customerPhone: "010-7890-1234",
    customerEmail: "minjae.kang@email.com",
    orderDate: "2026-03-27 15:00",
    pickupDate: "2026-04-04 12:00",
    outboundDate: "2026-04-02 09:00",
    inboundDate: "2026-04-12 15:00",
    items: [
      { id: "item11", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
    ],
    subtotal: 299000, discount: 0, totalAmount: 299000, paidAmount: 299000,
    paymentMethod: "신용카드",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260327-008",
  },
  {
    id: "9",
    orderNumber: "ON_20260328009",
    sequenceNumber: 9,
    customerName: "윤서준",
    customerPhone: "010-8901-2345",
    customerEmail: "seojun.yoon@email.com",
    orderDate: "2026-03-28 10:30",
    pickupDate: "2026-04-06 12:00",
    outboundDate: "2026-04-03 14:00",
    inboundDate: "2026-04-14 10:00",
    items: [
      { id: "item12", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002" },
    ],
    subtotal: 450000, discount: 30000, totalAmount: 420000, paidAmount: 420000,
    paymentMethod: "카카오페이",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260328-009",
  },
  {
    id: "10",
    orderNumber: "ON_20260329010",
    sequenceNumber: 10,
    customerName: "송하은",
    customerPhone: "010-1122-3344",
    customerEmail: "haeun.song@email.com",
    orderDate: "2026-03-29 09:00",
    pickupDate: "2026-04-08 12:00",
    outboundDate: "2026-04-04 11:00",
    inboundDate: "2026-04-16 16:00",
    items: [
      { id: "item13", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 2, unitPrice: 35000, totalPrice: 70000, barcode: undefined },
    ],
    subtotal: 70000, discount: 0, totalAmount: 70000, paidAmount: 70000,
    paymentMethod: "네이버페이",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260329-010",
  },

  // ── 4. Completed · 정상 (픽업일 전 입고 완료 → 픽업 완료) ──
  {
    id: "11",
    orderNumber: "ON_20260317011",
    sequenceNumber: 11,
    customerName: "임지우",
    customerPhone: "010-9012-3456",
    customerEmail: "jiwoo.lim@email.com",
    orderDate: "2026-03-17 11:00",
    pickupDate: "2026-03-30 12:00",
    outboundDate: "2026-03-22 10:00",
    inboundDate: "2026-03-26 09:00",
    items: [
      { id: "item14", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
      { id: "item15", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 1, unitPrice: 45000, totalPrice: 45000, barcode: "8892841003" },
    ],
    subtotal: 344000, discount: 20000, totalAmount: 324000, paidAmount: 324000,
    paymentMethod: "신용카드",
    status: "completed", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-29T14:30:00",
    pickupQrCode: "PU-20260317-011",
  },
  {
    id: "12",
    orderNumber: "ON_20260319012",
    sequenceNumber: 12,
    customerName: "오준혁",
    customerPhone: "010-2233-4455",
    customerEmail: "junhyuk.oh@email.com",
    orderDate: "2026-03-19 09:00",
    pickupDate: "2026-04-02 12:00",
    outboundDate: "2026-03-23 15:00",
    inboundDate: "2026-03-27 11:00",
    items: [
      { id: "item16", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001" },
    ],
    subtotal: 189000, discount: 0, totalAmount: 189000, paidAmount: 189000,
    paymentMethod: "토스",
    status: "completed", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-31T10:00:00",
    pickupQrCode: "PU-20260319-012",
  },
  {
    id: "13",
    orderNumber: "ON_20260322013",
    sequenceNumber: 13,
    customerName: "배수아",
    customerPhone: "010-3344-5566",
    customerEmail: "sua.bae@email.com",
    orderDate: "2026-03-22 14:00",
    pickupDate: "2026-04-04 12:00",
    outboundDate: "2026-03-26 09:00",
    inboundDate: "2026-03-30 14:00",
    items: [
      { id: "item17", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
      { id: "item18", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 1, unitPrice: 35000, totalPrice: 35000, barcode: undefined },
    ],
    subtotal: 334000, discount: 5000, totalAmount: 329000, paidAmount: 329000,
    paymentMethod: "카카오페이",
    status: "completed", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-04-03T11:20:00",
    pickupQrCode: "PU-20260322-013",
  },
  {
    id: "14",
    orderNumber: "ON_20260325014",
    sequenceNumber: 14,
    customerName: "권도윤",
    customerPhone: "010-4455-6677",
    customerEmail: "doyun.kwon@email.com",
    orderDate: "2026-03-25 16:00",
    pickupDate: "2026-04-06 12:00",
    outboundDate: "2026-03-28 10:00",
    inboundDate: "2026-04-01 09:00",
    items: [
      { id: "item19", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002" },
    ],
    subtotal: 450000, discount: 50000, totalAmount: 400000, paidAmount: 400000,
    paymentMethod: "신용카드",
    status: "completed", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-04-05T16:45:00",
    pickupQrCode: "PU-20260325-014",
  },

  // ── 5. Completed · 입고일 기준 (입고가 픽업일 이후, 입고일+14일 내 완료) ──
  {
    id: "15",
    orderNumber: "ON_20260312015",
    sequenceNumber: 15,
    customerName: "신예린",
    customerPhone: "010-5566-7788",
    customerEmail: "yerin.shin@email.com",
    orderDate: "2026-03-12 10:00",
    pickupDate: "2026-03-26 12:00",
    outboundDate: "2026-03-17 09:00",
    inboundDate: "2026-03-30 14:00",
    items: [
      { id: "item20", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001" },
      { id: "item21", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 1, unitPrice: 35000, totalPrice: 35000, barcode: undefined },
    ],
    subtotal: 224000, discount: 0, totalAmount: 224000, paidAmount: 224000,
    paymentMethod: "네이버페이",
    status: "completed", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-04-06T09:00:00",
    pickupQrCode: "PU-20260312-015",
  },

  // ── 6. Cancelled ──
  {
    id: "16",
    orderNumber: "ON_20260326016",
    sequenceNumber: 16,
    customerName: "문채원",
    customerPhone: "010-6677-8899",
    customerEmail: "chaewon.moon@email.com",
    orderDate: "2026-03-26 09:00",
    pickupDate: "2026-04-09 12:00",
    outboundDate: "2026-03-30 10:00",
    inboundDate: undefined,
    items: [
      { id: "item22", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098", disposition: "warehouse" as const },
    ],
    subtotal: 299000, discount: 0, totalAmount: 299000, paidAmount: 299000,
    paymentMethod: "신용카드",
    status: "cancelled", pickupStatus: "waiting", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-04-04T11:30:00",
    cancelReason: "Change of Mind",
    pickupQrCode: "PU-20260326-016",
  },
  {
    id: "17",
    orderNumber: "ON_20260327017",
    sequenceNumber: 17,
    customerName: "장현우",
    customerPhone: "010-7788-9900",
    customerEmail: "hyunwoo.jang@email.com",
    orderDate: "2026-03-27 14:00",
    pickupDate: "2026-04-12 12:00",
    outboundDate: "2026-04-01 09:00",
    inboundDate: "2026-04-06 15:00",
    items: [
      { id: "item23", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002", disposition: "store" as const },
      { id: "item24", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 1, unitPrice: 45000, totalPrice: 45000, barcode: "8892841003" },
    ],
    subtotal: 495000, discount: 25000, totalAmount: 470000, paidAmount: 470000,
    paymentMethod: "토스",
    status: "cancelled", pickupStatus: "ready", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-04-27T09:00:00",
    cancelReason: "Change Payment Method",
    pickupQrCode: "PU-20260327-017",
  },
  {
    id: "23",
    orderNumber: "ON_20260318023",
    sequenceNumber: 23,
    customerName: "양서윤",
    customerPhone: "010-4466-8800",
    customerEmail: "seoyun.yang@email.com",
    orderDate: "2026-03-18 10:30",
    pickupDate: "2026-04-01 12:00",
    outboundDate: "2026-03-22 09:00",
    inboundDate: "2026-03-28 11:00",
    items: [
      { id: "item33", productName: "URBAN WALKER-X", sku: "2200100", quantity: 2, unitPrice: 189000, totalPrice: 378000, barcode: "8892840001", disposition: "store" as const },
    ],
    subtotal: 378000, discount: 0, totalAmount: 378000, paidAmount: 378000,
    paymentMethod: "카카오페이",
    status: "cancelled", pickupStatus: "ready", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-04-02T14:00:00",
    cancelReason: "Purchase Different Product",
    pickupQrCode: "PU-20260318-023",
  },
  {
    id: "24",
    orderNumber: "ON_20260320024",
    sequenceNumber: 24,
    customerName: "홍예준",
    customerPhone: "010-5577-9911",
    customerEmail: "yejun.hong@email.com",
    orderDate: "2026-03-20 15:00",
    pickupDate: "2026-04-05 12:00",
    outboundDate: "2026-03-24 10:00",
    inboundDate: undefined,
    items: [
      { id: "item34", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
      { id: "item35", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 1, unitPrice: 35000, totalPrice: 35000, barcode: undefined },
    ],
    subtotal: 334000, discount: 5000, totalAmount: 329000, paidAmount: 329000,
    paymentMethod: "네이버페이",
    status: "cancelled", pickupStatus: "waiting", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-03-28T09:00:00",
    cancelReason: "Change of Mind",
    pickupQrCode: "PU-20260320-024",
  },
  {
    id: "25",
    orderNumber: "ON_20260322025",
    sequenceNumber: 25,
    customerName: "차은우",
    customerPhone: "010-6688-0022",
    customerEmail: "eunwoo.cha@email.com",
    orderDate: "2026-03-22 09:00",
    pickupDate: "2026-04-08 12:00",
    outboundDate: "2026-03-26 11:00",
    inboundDate: "2026-03-30 14:00",
    items: [
      { id: "item36", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002", disposition: "warehouse" as const },
    ],
    subtotal: 450000, discount: 30000, totalAmount: 420000, paidAmount: 420000,
    paymentMethod: "신용카드",
    status: "cancelled", pickupStatus: "ready", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-04-06T16:30:00",
    cancelReason: "Change of Mind",
    pickupQrCode: "PU-20260322-025",
  },
  {
    id: "26",
    orderNumber: "ON_20260325026",
    sequenceNumber: 26,
    customerName: "나은서",
    customerPhone: "010-7799-1133",
    customerEmail: "eunseo.na@email.com",
    orderDate: "2026-03-25 12:00",
    pickupDate: "2026-04-10 12:00",
    outboundDate: "2026-03-29 09:00",
    inboundDate: "2026-04-03 10:00",
    items: [
      { id: "item37", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 3, unitPrice: 45000, totalPrice: 135000, barcode: "8892841003" },
    ],
    subtotal: 135000, discount: 0, totalAmount: 135000, paidAmount: 135000,
    paymentMethod: "토스",
    status: "cancelled", pickupStatus: "ready", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-04-07T11:00:00",
    cancelReason: "Change Payment Method",
    pickupQrCode: "PU-20260325-026",
  },

  {
    id: "27",
    orderNumber: "ON_20260315027",
    sequenceNumber: 27,
    customerName: "윤하영",
    customerPhone: "010-1122-5566",
    customerEmail: "hayoung.yoon@email.com",
    orderDate: "2026-03-15 09:00",
    pickupDate: "2026-03-28 12:00",
    outboundDate: "2026-03-19 10:00",
    inboundDate: "2026-03-24 11:00",
    items: [
      { id: "item38", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098", disposition: "store" as const },
    ],
    subtotal: 299000, discount: 0, totalAmount: 299000, paidAmount: 299000,
    paymentMethod: "신용카드",
    status: "cancelled", pickupStatus: "ready", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-03-27T10:00:00",
    cancelReason: "Change of Mind",
    pickupQrCode: "PU-20260315-027",
  },
  {
    id: "28",
    orderNumber: "ON_20260316028",
    sequenceNumber: 28,
    customerName: "백승호",
    customerPhone: "010-2233-6677",
    customerEmail: "seungho.baek@email.com",
    orderDate: "2026-03-16 14:00",
    pickupDate: "2026-04-01 12:00",
    outboundDate: "2026-03-20 09:00",
    inboundDate: "2026-03-25 14:00",
    items: [
      { id: "item39", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002" },
      { id: "item40", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 1, unitPrice: 45000, totalPrice: 45000, barcode: "8892841003", disposition: "warehouse" as const },
    ],
    subtotal: 495000, discount: 15000, totalAmount: 480000, paidAmount: 480000,
    paymentMethod: "토스",
    status: "cancelled", pickupStatus: "ready", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-03-30T16:00:00",
    cancelReason: "Purchase Different Product",
    pickupQrCode: "PU-20260316-028",
  },
  {
    id: "29",
    orderNumber: "ON_20260319029",
    sequenceNumber: 29,
    customerName: "고은비",
    customerPhone: "010-3344-7788",
    customerEmail: "eunbi.go@email.com",
    orderDate: "2026-03-19 11:00",
    pickupDate: "2026-04-03 12:00",
    outboundDate: "2026-03-23 10:00",
    inboundDate: undefined,
    items: [
      { id: "item41", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 2, unitPrice: 35000, totalPrice: 70000, barcode: undefined },
    ],
    subtotal: 70000, discount: 0, totalAmount: 70000, paidAmount: 70000,
    paymentMethod: "네이버페이",
    status: "cancelled", pickupStatus: "waiting", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-03-25T11:00:00",
    cancelReason: "Change Payment Method",
    pickupQrCode: "PU-20260319-029",
  },
  {
    id: "30",
    orderNumber: "ON_20260321030",
    sequenceNumber: 30,
    customerName: "임도현",
    customerPhone: "010-4455-8899",
    customerEmail: "dohyun.lim@email.com",
    orderDate: "2026-03-21 16:00",
    pickupDate: "2026-04-06 12:00",
    outboundDate: "2026-03-25 09:00",
    inboundDate: "2026-03-29 15:00",
    items: [
      { id: "item42", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001", disposition: "warehouse" as const },
      { id: "item43", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
    ],
    subtotal: 488000, discount: 20000, totalAmount: 468000, paidAmount: 468000,
    paymentMethod: "카카오페이",
    status: "cancelled", pickupStatus: "ready", approvalStatus: "reject", processingStatus: "cancelled",
    cancelledAt: "2026-04-05T09:30:00",
    cancelReason: "Other",
    pickupQrCode: "PU-20260321-030",
  },

  // ── 7. Refunded ──
  {
    id: "18",
    orderNumber: "ON_20260315018",
    sequenceNumber: 18,
    customerName: "조은서",
    customerPhone: "010-8899-0011",
    customerEmail: "eunseo.jo@email.com",
    orderDate: "2026-03-15 11:00",
    pickupDate: "2026-03-27 12:00",
    outboundDate: "2026-03-19 10:00",
    inboundDate: "2026-03-25 14:00",
    items: [
      { id: "item25", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
    ],
    subtotal: 299000, discount: 0, totalAmount: 299000, paidAmount: 299000,
    paymentMethod: "카카오페이",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-26T15:00:00",
    returnedAt: "2026-04-04T10:00:00",
    returnReason: "Product Defect",
    returnGrading: "resellable",
    inventoryLocation: "omni",
    pickupQrCode: "PU-20260315-018",
  },
  {
    id: "31",
    orderNumber: "ON_20260310031",
    sequenceNumber: 31,
    customerName: "정민호",
    customerPhone: "010-5566-9900",
    customerEmail: "minho.jung@email.com",
    orderDate: "2026-03-10 10:00",
    pickupDate: "2026-03-24 12:00",
    outboundDate: "2026-03-14 09:00",
    inboundDate: "2026-03-20 11:00",
    items: [
      { id: "item44", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001", grading: "sellable" as const, disposition: "store" as const, outboundNo: "TOO01026" },
    ],
    subtotal: 189000, discount: 0, totalAmount: 189000, paidAmount: 189000,
    paymentMethod: "신용카드",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-23T14:00:00",
    returnedAt: "2026-04-01T09:00:00",
    pickupQrCode: "PU-20260310-031",
  },
  {
    id: "32",
    orderNumber: "ON_20260311032",
    sequenceNumber: 32,
    customerName: "한수빈",
    customerPhone: "010-6677-0011",
    customerEmail: "subin.han@email.com",
    orderDate: "2026-03-11 14:00",
    pickupDate: "2026-03-25 12:00",
    outboundDate: "2026-03-15 10:00",
    inboundDate: "2026-03-21 14:00",
    items: [
      { id: "item45", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002", grading: "resellable" as const, disposition: "warehouse" as const, outboundNo: "TOO01026" },
      { id: "item46", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 1, unitPrice: 45000, totalPrice: 45000, barcode: "8892841003", grading: "sellable" as const, disposition: "store" as const, outboundNo: "TOO01028" },
    ],
    subtotal: 495000, discount: 20000, totalAmount: 475000, paidAmount: 475000,
    paymentMethod: "카카오페이",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-24T10:00:00",
    returnedAt: "2026-04-02T11:00:00",
    pickupQrCode: "PU-20260311-032",
  },
  {
    id: "33",
    orderNumber: "ON_20260313033",
    sequenceNumber: 33,
    customerName: "오지윤",
    customerPhone: "010-7788-1122",
    customerEmail: "jiyun.oh@email.com",
    orderDate: "2026-03-13 09:00",
    pickupDate: "2026-03-27 12:00",
    outboundDate: "2026-03-17 11:00",
    inboundDate: "2026-03-23 09:00",
    items: [
      { id: "item47", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098", grading: "dispose" as const, disposition: "warehouse" as const, outboundNo: "TOO01027" },
    ],
    subtotal: 299000, discount: 10000, totalAmount: 289000, paidAmount: 289000,
    paymentMethod: "토스",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-26T15:00:00",
    returnedAt: "2026-04-03T14:00:00",
    pickupQrCode: "PU-20260313-033",
  },
  {
    id: "34",
    orderNumber: "ON_20260314034",
    sequenceNumber: 34,
    customerName: "김하늘",
    customerPhone: "010-8899-2233",
    customerEmail: "haneul.kim@email.com",
    orderDate: "2026-03-14 11:30",
    pickupDate: "2026-03-28 12:00",
    outboundDate: "2026-03-18 10:00",
    inboundDate: "2026-03-24 13:00",
    items: [
      { id: "item48", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 1, unitPrice: 35000, totalPrice: 35000, barcode: undefined, grading: "sellable" as const },
      { id: "item49", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001", grading: "resellable" as const },
    ],
    subtotal: 224000, discount: 0, totalAmount: 224000, paidAmount: 224000,
    paymentMethod: "네이버페이",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-27T11:00:00",
    returnedAt: "2026-04-04T16:00:00",
    pickupQrCode: "PU-20260314-034",
  },
  {
    id: "35",
    orderNumber: "ON_20260316035",
    sequenceNumber: 35,
    customerName: "박시우",
    customerPhone: "010-9900-3344",
    customerEmail: "siwoo.park@email.com",
    orderDate: "2026-03-16 15:00",
    pickupDate: "2026-03-30 12:00",
    outboundDate: "2026-03-20 09:00",
    inboundDate: "2026-03-26 10:00",
    items: [
      { id: "item50", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002", grading: "resellable" as const, disposition: "store" as const, outboundNo: "TOO01028" },
    ],
    subtotal: 450000, discount: 30000, totalAmount: 420000, paidAmount: 420000,
    paymentMethod: "신용카드",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-29T09:00:00",
    returnedAt: "2026-04-05T10:00:00",
    pickupQrCode: "PU-20260316-035",
  },
  {
    id: "36",
    orderNumber: "ON_20260317036",
    sequenceNumber: 36,
    customerName: "송유나",
    customerPhone: "010-0011-4455",
    customerEmail: "yuna.song@email.com",
    orderDate: "2026-03-17 10:00",
    pickupDate: "2026-03-31 12:00",
    outboundDate: "2026-03-21 11:00",
    inboundDate: "2026-03-27 14:00",
    items: [
      { id: "item51", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098", grading: "sellable" as const },
      { id: "item52", productName: "PACKAGE SET(BOLD)", sku: "11403903", quantity: 1, unitPrice: 35000, totalPrice: 35000, barcode: undefined, grading: "dispose" as const },
    ],
    subtotal: 334000, discount: 5000, totalAmount: 329000, paidAmount: 329000,
    paymentMethod: "토스",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-30T14:00:00",
    returnedAt: "2026-04-06T11:00:00",
    pickupQrCode: "PU-20260317-036",
  },
  {
    id: "37",
    orderNumber: "ON_20260318037",
    sequenceNumber: 37,
    customerName: "최준서",
    customerPhone: "010-1122-5577",
    customerEmail: "junseo.choi@email.com",
    orderDate: "2026-03-18 13:00",
    pickupDate: "2026-04-01 12:00",
    outboundDate: "2026-03-22 09:00",
    inboundDate: "2026-03-28 11:00",
    items: [
      { id: "item53", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001", grading: "dispose" as const },
    ],
    subtotal: 189000, discount: 0, totalAmount: 189000, paidAmount: 189000,
    paymentMethod: "카카오페이",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-03-31T16:00:00",
    returnedAt: "2026-04-07T09:00:00",
    pickupQrCode: "PU-20260318-037",
  },
  {
    id: "38",
    orderNumber: "ON_20260320038",
    sequenceNumber: 38,
    customerName: "강예진",
    customerPhone: "010-2233-6688",
    customerEmail: "yejin.kang@email.com",
    orderDate: "2026-03-20 09:30",
    pickupDate: "2026-04-03 12:00",
    outboundDate: "2026-03-24 10:00",
    inboundDate: "2026-03-30 09:00",
    items: [
      { id: "item54", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 2, unitPrice: 45000, totalPrice: 90000, barcode: "8892841003", grading: "sellable" as const },
    ],
    subtotal: 90000, discount: 0, totalAmount: 90000, paidAmount: 90000,
    paymentMethod: "네이버페이",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-04-02T11:00:00",
    returnedAt: "2026-04-08T14:00:00",
    pickupQrCode: "PU-20260320-038",
  },
  {
    id: "39",
    orderNumber: "ON_20260322039",
    sequenceNumber: 39,
    customerName: "문서연",
    customerPhone: "010-3344-7799",
    customerEmail: "seoyeon.moon@email.com",
    orderDate: "2026-03-22 14:00",
    pickupDate: "2026-04-05 12:00",
    outboundDate: "2026-03-26 09:00",
    inboundDate: "2026-04-01 14:00",
    items: [
      { id: "item55", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098", grading: "resellable" as const },
      { id: "item56", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002", grading: "sellable" as const },
      { id: "item57", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 1, unitPrice: 45000, totalPrice: 45000, barcode: "8892841003", grading: "dispose" as const },
    ],
    subtotal: 794000, discount: 50000, totalAmount: 744000, paidAmount: 744000,
    paymentMethod: "신용카드",
    status: "returned", pickupStatus: "completed", approvalStatus: "confirm", processingStatus: "completed",
    completedAt: "2026-04-04T10:00:00",
    returnedAt: "2026-04-09T09:00:00",
    pickupQrCode: "PU-20260322-039",
  },

  // ── 8. 추가 다양한 케이스 ──
  // Ready · 입고 완료, 아이템 3개
  {
    id: "19",
    orderNumber: "ON_20260404019",
    sequenceNumber: 19,
    customerName: "류하린",
    customerPhone: "010-9900-1122",
    customerEmail: "harin.ryu@email.com",
    orderDate: "2026-04-04 09:00",
    pickupDate: "2026-04-26 12:00",
    outboundDate: "2026-04-08 10:00",
    inboundDate: "2026-04-12 11:00",
    items: [
      { id: "item26", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
      { id: "item27", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001" },
      { id: "item28", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 2, unitPrice: 45000, totalPrice: 90000, barcode: "8892841003" },
    ],
    subtotal: 578000, discount: 40000, totalAmount: 538000, paidAmount: 538000,
    paymentMethod: "신용카드",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260404-019",
  },
  // Ready · 픽업일 기준, 단일 고가 상품
  {
    id: "20",
    orderNumber: "ON_20260402020",
    sequenceNumber: 20,
    customerName: "황지민",
    customerPhone: "010-0011-2233",
    customerEmail: "jimin.hwang@email.com",
    orderDate: "2026-04-02 15:00",
    pickupDate: "2026-05-03 12:00",
    outboundDate: "2026-04-06 09:00",
    inboundDate: "2026-04-10 14:00",
    items: [
      { id: "item29", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002" },
    ],
    subtotal: 450000, discount: 0, totalAmount: 450000, paidAmount: 450000,
    paymentMethod: "네이버페이",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260402-020",
  },

  // ── 9. Auto-cancel 임박 케이스 ──
  // 정상 입고 (입고일 ≤ 픽업일) → 마감 = 픽업일+14 = 04/09, D-1
  {
    id: "21",
    orderNumber: "ON_20260312021",
    sequenceNumber: 21,
    customerName: "김태형",
    customerPhone: "010-2244-6688",
    customerEmail: "taehyung.kim@email.com",
    orderDate: "2026-03-12 09:00",
    pickupDate: "2026-03-26 12:00",
    outboundDate: "2026-03-15 10:00",
    inboundDate: "2026-03-24 10:00",
    items: [
      { id: "item30", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
    ],
    subtotal: 299000, discount: 0, totalAmount: 299000, paidAmount: 299000,
    paymentMethod: "신용카드",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260310-021",
  },
  // 지연 입고 (입고일 > 픽업일) → 마감 = 입고일+15 = 04/10, D-2
  {
    id: "22",
    orderNumber: "ON_20260310022",
    sequenceNumber: 22,
    customerName: "서지안",
    customerPhone: "010-3355-7799",
    customerEmail: "jian.seo@email.com",
    orderDate: "2026-03-10 14:00",
    pickupDate: "2026-03-20 12:00",
    outboundDate: "2026-03-14 09:00",
    inboundDate: "2026-03-26 14:00",
    items: [
      { id: "item31", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002" },
      { id: "item32", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 1, unitPrice: 45000, totalPrice: 45000, barcode: "8892841003" },
    ],
    subtotal: 495000, discount: 10000, totalAmount: 485000, paidAmount: 485000,
    paymentMethod: "카카오페이",
    status: "ready", pickupStatus: "ready", approvalStatus: "unready", processingStatus: "pending",
    pickupQrCode: "PU-20260305-022",
  },
]

export const mockReturnRequests: Record<string, ReturnRequest> = {
  "RTN-1001": {
    returnQrCode: "RTN-1001",
    orderId: "11",
    orderNumber: "ON_20260317011",
    customerName: "임지우",
    items: [
      { id: "item14", productName: "SMART ALIO-01", sku: "1100000", quantity: 1, unitPrice: 299000, totalPrice: 299000, barcode: "8892839098" },
      { id: "item15", productName: "CARE KIT PREMIUM", sku: "3300201", quantity: 1, unitPrice: 45000, totalPrice: 45000, barcode: "8892841003" },
    ],
    refundAmount: 324000,
    reason: "Product defect",
    pickupStatus: "completed",
    orderDate: "2026-03-17 11:00",
    completedAt: "2026-03-29T14:30:00",
  },
  "RTN-2024-000456": {
    returnQrCode: "RTN-2024-000456",
    orderId: "14",
    orderNumber: "ON_20260325014",
    customerName: "권도윤",
    items: [
      { id: "item19", productName: "CLASSIC LEATHER BAG", sku: "3300200", quantity: 1, unitPrice: 450000, totalPrice: 450000, barcode: "8892841002" },
    ],
    refundAmount: 400000,
    reason: "Color dissatisfaction",
    pickupStatus: "completed",
    orderDate: "2026-03-25 16:00",
    completedAt: "2026-04-05T16:45:00",
  },
  "RTN-2024-000457": {
    returnQrCode: "RTN-2024-000457",
    orderId: "15",
    orderNumber: "ON_20260312015",
    customerName: "신예린",
    items: [
      { id: "item20", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001" },
    ],
    refundAmount: 224000,
    reason: "Changed mind",
    pickupStatus: "completed",
    orderDate: "2026-03-12 10:00",
    completedAt: "2026-04-06T09:00:00",
  },
  "RTN-1002": {
    returnQrCode: "RTN-1002",
    orderId: "12",
    orderNumber: "ON_20260319012",
    customerName: "오준혁",
    items: [
      { id: "item16-1", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001" },
      { id: "item16-2", productName: "URBAN WALKER-X", sku: "2200100", quantity: 1, unitPrice: 189000, totalPrice: 189000, barcode: "8892840001" },
    ],
    refundAmount: 378000,
    reason: "Size exchange",
    pickupStatus: "completed",
    orderDate: "2026-03-19 09:00",
    completedAt: "2026-03-31T10:00:00",
  },
}
